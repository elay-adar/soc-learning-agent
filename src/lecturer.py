...
def _stage_rules(
    ...
    if key == WHY_POSSIBLE_KEY:
        ...
        return (
            "- Stage 2 is the only in-depth technical explanation. Cover, in order: the anchor; the component's normal function; the root cause (which security assumption broke); the failure mechanism step by step, meaning how the design flaw works: what the system does, what it fails to check, and what that lets anyone obtain. Do not list the attacker's procedure here, do not number attack steps, and do not write blocks that start with 'Step 1', 'Step 2' and so on: the attacker's steps belong to stage 3; the preconditions; and what a fix or hardening changes. "
...
- Glossary: every term you define goes in the "glossary" list as {{"term", "definition"}}. The definition is ONE sentence of at most 200 characters, tagged like a block (documented with a source_url from the Pack, inference, or unknown). Do not define terms inside blocks: define them in the glossary, then just use them in blocks. Terms already defined in earlier stages are listed in the user message: use them, never define them again. Never define the same term twice in one stage. Define a term in the glossary of the first stage whose text uses it. Do not use a specialised term, for example an algorithm name, in a stage before the stage that defines it. A stage that introduces no new specific term may leave the glossary empty.
- Every sentence in a documented or secondary block must be supported by the entries the Pack records under that URL. Do not add an explanation, consequence or comparison of your own: put it in its own block tagged inference. A sentence that only points back to an earlier stage ("as explained in stage 2") also goes in an inference block. Code rejects a documented sentence that shares almost no words with the entries under its URL.
