...
- Stage rules, `config/stages.toml` and the Lecturer prompt follow D-029 (secondary tag checks, "no CVE or CWE" statement, technique topics skip the exploitation rules; D-032): built and unit-tested, not yet run live. Next: the Researcher's technique path.
- Technique-only input path for the Researcher (`collect_technique_facts`, `run_researcher` with a technique id and `--source-url` pages, exploitation status `not_applicable`; D-033): built and unit-tested. First live run on Kerberoasting (T1558.003) completed on 2026-09-26 with no corrections; the manual check of its fields against the two pages is still open. D-034 then limited secondary sources to mechanism and steps and removed step-level ATT&CK mapping from secondary steps, so the technique Pack needs a re-run before stages are built from it.
- `run_stages.py` and `serve_stages.py` accept a technique id (`src/topic.py`, D-035), and the trace check samples secondary claims too. Built and unit-tested; stages 1 to 3 have not been run live on a technique Pack yet. Researcher re-run on T1558.003 passed the D-034 rules on 2026-09-26; the user is doing the manual check of its fields.
- First live stages 1 to 3 run on T1558.003 passed the rules on 2026-09-26. D-036 then tightened stage 1 and 2 (no algorithm names in stage 1, no restated attack steps in stage 2), the Lecturer wording, and added a warn-only support check (`src/support_check.py`). Built and unit-tested; a fresh live re-run to check the improved output is next.
- Second live stages run on T1558.003 (after D-036) gave clearly better output. D-037 rejects a documented sentence with under 30% word overlap with its source and records two accepted limitations of secondary text (an added consequence under CrowdStrike, background from memory under Picus). Built and unit-tested; not yet re-run live.
