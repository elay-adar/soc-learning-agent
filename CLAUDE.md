...
- Technique-only input path for the Researcher (`collect_technique_facts`, `run_researcher` with a technique id and `--source-url` pages, exploitation status `not_applicable`; D-033): built and unit-tested, not yet run live. Next: a live run on Kerberoasting (T1558.003).
